...
things organized.
*/
function jb_add_extra_contactmethod( $contactmethods ) {
    // Add new ones
...
add_filter('user_contactmethods','jb_add_extra_contactmethod',10,1);

function jb_add_custom_user_profile_fields( $user ) {
?>
	<h3><?php _e('Extra Profile Information', 'jb'); ?></h3>
	<table class="form-table">
		<tr>
			<th>
				<label for="address"><?php _e('Address', 'jb'); ?>
			</label></th>
			<td>
				<textarea name="address" id="address" rows="5" cols="30" class="regular-text" /><?php echo esc_attr( get_the_author_meta( 'address', $user->ID ) ); ?></textarea><br />
				<span class="description"><?php _e('Please enter your address.', 'jb'); ?></span>
			</td>
		</tr>
		<tr>
			<th>
				<label for="address"><?php _e('Prefix', 'jb'); ?>
			</label></th>
			<td>
				<input type="text" name="prefix" id="prefix" value="<?php echo esc_attr( get_the_author_meta( 'prefix', $user->ID ) ); ?>" class="regular-text" />
			</td>
		</tr>
		<tr>
			<th>
				<label for="address"><?php _e('Suffix', 'jb'); ?>
			</label></th>
			<td>
				<input type="text" name="suffix" id="suffix" value="<?php echo esc_attr( get_the_author_meta( 'suffix', $user->ID ) ); ?>" class="regular-text" />
			</td>
		</tr>
	</table>
<?php }
function jb_save_custom_user_profile_fields( $user_id ) {
	if ( !current_user_can( 'edit_user', $user_id ) )
		return FALSE;
	update_usermeta( $user_id, 'address', $_POST['address'] );
	update_usermeta( $user_id, 'prefix', $_POST['prefix'] );
	update_usermeta( $user_id, 'suffix', $_POST['suffix'] );
}
add_action( 'show_user_profile', 'jb_add_custom_user_profile_fields' );
add_action( 'edit_user_profile', 'jb_add_custom_user_profile_fields' );
add_action( 'personal_options_update', 'jb_save_custom_user_profile_fields' );
add_action( 'edit_user_profile_update', 'jb_save_custom_user_profile_fields' );
