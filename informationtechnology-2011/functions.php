...
define( 'CHILD_THEME_NAME', 'Information Technology Theme' );
define( 'CHILD_THEME_URL', 'http://instructionaltechnology.wooster.edu/themes/informationtechnology' );
...
add_theme_support( 'genesis-custom-header', array( 'width' => 960, 'height' => 90, 'textcolor' => 'ffffff', 'admin_header_callback' => 'informationtechnology_admin_style' ) );
 ...
 *
 */
function informationtechnology_admin_style() {
...
function remove_classroom_taxonomy() {
	...
	remove_meta_box( 'tagsdiv-seating', 'itclassroom', 'side' );
	remove_meta_box( 'tagsdiv-style', 'itclassroom', 'side' );
	remove_meta_box( 'tagsdiv-hardware', 'itclassroom', 'side' );
	remove_meta_box( 'tagsdiv-specialized', 'itclassroom', 'side' );
	remove_meta_box( 'tagsdiv-other', 'itclassroom', 'side' );
	remove_meta_box( 'tagsdiv-software', 'itclassroom', 'side' );
}

add_filter( 'manage_itpeople_posts_columns', 'it_itpeople_columns' );
add_action('manage_itpeople_posts_custom_column', 'it_itpeople_custom_column', 10, 2);
function it_itpeople_columns($defaults) {
    $defaults['role'] = 'Role';
    return $defaults;
}
function it_itpeople_custom_column($column_name, $post_id) {
...
add_filter( 'manage_itclassroom_posts_columns', 'it_itclassroom_columns' );
add_action('manage_itclassroom_posts_custom_column', 'it_itclassroom_custom_column', 10, 2);
function it_itclassroom_columns($defaults) {
    $defaults['building'] = 'Building';
		$defaults['seating'] = 'Seating Capacity';
		$defaults['style'] = 'Classroom Style';
		$defaults['hardware'] = 'Installed Hardware';
		$defaults['specialized'] = 'Specialized Hardware';
		$defaults['software'] = 'Installed Software';
		$defaults['other'] = 'Other Features';
    return $defaults;
}
function it_itclassroom_custom_column($column_name, $post_id) {
    $taxonomy = $column_name;
    $post_type = get_post_type($post_id);
    $terms = get_the_terms($post_id, $taxonomy);
...
/**
 * A function to take a "messy" string stored in the database of a Term ID, and
 * return the Term Name
 *
 * @param string $custom_field
 * @param string $taxonomy
 * @param int $post_id Optional, only needed if called from outside of The Loop
 * @return string
 */
function it_get_term_name( $custom_field, $taxonomy, $post_id = null ) {
    if( !$post_id )
        $post_id = get_the_ID();

    $raw_term = maybe_unserialize( get_post_meta( $post_id, $custom_field, true ) );
    $term = get_term( $raw_term[0], $taxonomy );
	...
	$meta_boxes[] = array(
		...
		'fields' => array(
			...
			array(
				...
				'taxonomy' => 'seating', //Enter Taxonomy Slug
				'type' => 'taxonomy_radio',	
			),
			array(
			...
			array(
				...
				'taxonomy' => 'hardware', //Enter Taxonomy Slug
				'type' => 'taxonomy_multicheck',	
			),
			array(
				...
				'taxonomy' => 'specialized', //Enter Taxonomy Slug
				'type' => 'taxonomy_multicheck',	
			),
			array(
				'name' => 'Other Features',
				...
				'taxonomy' => 'other', //Enter Taxonomy Slug
				'type' => 'taxonomy_multicheck',	
			),
			array(
				...
				'taxonomy' => 'software', //Enter Taxonomy Slug
				'type' => 'taxonomy_multicheck',	
			),
		),
	);
